...
# This need to be changed to you actual pi-hole dir on you machine
hostPiHoleDir='/home/pi/rhaspy-deploy-public/pi-hole/etc-pihole'
dockerPiHoleDir='/etc/pihole'

# This need to be changed to your actual repo dir on your machine
repoDir='/home/pi/youTube_ads_4_pi-hole'

blackListFile="$hostPiHoleDir/black.list"
blacklist="$hostPiHoleDir/blacklist.txt"

dockerBlacklist="$dockerPiHoleDir/blacklist.txt"
...
while read ignoredDns ; do /usr/bin/sqlite3 $hostPiHoleDir/gravity.db "delete from domainlist where domain like '%$ignoredDns%' " ; done < ignore.list
	
## adding it to the blacklist in Pihole V5 
# only 200 Domains at once
docker exec -it pihole xargs -a $dockerBlacklist -L200 pihole -b -nr
# restart dns  
...
#sharedlogs=`sudo /usr/bin/sqlite3 ${piHoleDir}/pihole-FTL.db "select domain from queries where domain like '%googlevideo.com'" |uniq -d |tr '\n' ','`
